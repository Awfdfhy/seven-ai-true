...
const releaseTests=fs.readdirSync(releaseDir).filter(name=>name.endsWith('.test.cjs')).sort().map(name=>path.join('release',name));
releaseTests.push(path.join('release','static-audit.cjs'),path.join('release','release-verify.cjs'));
