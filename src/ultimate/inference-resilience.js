...
class ProviderResilienceController{
 ...
 assertAttempt(providerId,input={}){check(this.canAttempt(providerId,input),'PROVIDER_TEMPORARILY_UNAVAILABLE');return this.snapshot(providerId);}
 finish(providerId,input={}){let quotaError=null;try{this.quotas.consume(providerId,{requests:1,tokens:input.estimatedTokens||0,now:input.now});}catch(e){quotaError=String(e&&e.message||e);}const circuit=this.breakers.record(providerId,input);return{circuit,quota:this.quotas.snapshot(providerId,input.now),quotaError};}
...
class AdvancedModelExecutor{
 ...
 async execute(input={}){let plan=input.plan||this.control.plan(input);plan=this._filteredPlan(plan,input);const freshness=this.freshness.allowCache(input),selected=plan.route.selected;let cacheKey=null;if(freshness.allow){cacheKey=this.cache.key({namespace:input.cacheNamespace,modelId:selected.modelId,messages:input.messages,prompt:input.prompt,options:input.options,contextRevision:input.contextRevision,toolRevision:input.toolRevision,promptProgramVersion:input.promptProgramVersion});const hit=this.cache.get(cacheKey);if(hit){if(input.onChunk&&hit.output)input.onChunk(hit.output,{providerId:selected.providerId,modelId:selected.modelId,cacheHit:true});return{status:'completed',plan,selected,attempts:[],run:{id:`cache:${cacheKey}`,status:'completed',output:hit.output,events:[{type:'cache_hit',data:{key:cacheKey},at:Date.now()}]},cache:{hit:true,key:cacheKey}};}}const result=await this.base.execute({...input,plan});for(const a of result.attempts||[])this.resilience.finish(a.providerId,{success:a.status==='completed',errorClass:a.status,estimatedTokens:input.estimatedTokens||0});if(result.status==='completed'&&freshness.allow&&cacheKey&&result.run?.output!=null)this.cache.set(cacheKey,{output:result.run.output,cacheMeta:{namespace:input.cacheNamespace||'default',modelId:result.selected?.modelId||selected.modelId}},{ttlMs:freshness.ttlMs});return{...result,cache:{hit:false,key:cacheKey}};}
}
