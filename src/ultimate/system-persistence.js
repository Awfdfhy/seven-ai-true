(function(root,factory){
 const deps=(typeof module==='object'&&module.exports)?{P:require('./persistence.js'),Tools:require('./tool-fabric.js'),Parallel:require('./parallel-story.js')}:{P:root.SevenUltimatePersistence,Tools:root.SevenUltimateTools,Parallel:root.SevenUltimateParallelStory};const api=factory(deps);if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.SevenUltimateSystemPersistence=api;
...
const entries=m=>[...m.entries()].map(([k,v])=>[k,clone(v)]);const map=e=>new Map((e||[]).map(([k,v])=>[k,clone(v)]));
...
function hydrateSystem(os,b){
 ...
 os.characters.profiles=map(m.characters);os.story.timelines=new Map((m.story||[]).map(([id,s])=>[id,{series:clone(s.series),seasons:map(s.seasons),arcs:map(s.arcs),episodes:clone(s.episodes||[])}]));
 os.narrative.threads.threads=map(m.narrative?.threads);os.narrative.promises.promises=map(m.narrative?.promises);os.narrative.foreshadow.seeds=map(m.narrative?.seeds);os.narrative.pacing.scenes=clone(m.narrative?.pacingScenes||[]);os.genesis.records=map(m.genesis);os.spotlight.rows=map(m.spotlight);
 os.parallelStories.contracts=new Map((m.parallel?.contracts||[]).map(([k,c])=>[k,new D.Parallel.IsolationContract(c)]));os.parallelStories.storylets.storylets=map(m.parallel?.storylets);os.parallelStories.storylets.history=clone(m.parallel?.history||[]);
 os.models.providers=map(m.providers);os.models.models=map(m.models);os.models.outcomes=map(m.outcomes);os.intelligence.models=os.models;os.permissions.grants=map(m.permissionGrants);for(const t of m.customTools||[])if(!os.tools.tools.has(t.id))os.tools.register(t);
 ...
 os.visuals.characters=new Map((m.visuals?.characters||[]).map(([k,c])=>[k,{...clone(c),outfits:map(c.outfits),accessories:map(c.accessories)}]));os.visuals.images=map(m.visuals?.images);os.knowledge.rows=map(m.knowledge);os.relationships.edges=map(m.relationships?.edges);os.relationships.history=clone(m.relationships?.history||[]);os.plots.rows=map(m.plots);os.world.timelines=map(m.world);
