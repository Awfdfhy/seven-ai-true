...
class AdvancedCapabilityScheduler{
 ...
 async _executeNode(node,input){const run=async signal=>this.runtime.execute({...clone(node),capabilityId:node.capabilityId,signal,scope:node.scope||input.scope||'*',taskId:node.taskId||input.taskId,inputLabels:node.inputLabels||input.inputLabels,workload:node.workload||input.workload,explicitConfirmation:node.explicitConfirmation??input.explicitConfirmation});return this.bulkheads.run(this._bulkheadKey(node),()=>this.watchdog.run(run,{timeoutMs:node.timeoutMs||input.timeoutMs,signal:input.signal}),input.signal).catch(error=>({state:input.signal?.aborted?'cancelled':'failed',reason:String(error?.code||error?.message||error),error:String(error?.message||error)}));}
 async run(nodes=[],input={}){const runId=input.runId||`dag_${now()}_${Math.random().toString(36).slice(2)}`,dag=nodes instanceof ExecutionDAG?nodes:new ExecutionDAG(nodes),planner=new CriticalPathPlanner({latencyOf:id=>this._latency(id)}),rank=planner.rank(dag),pending=new Map(dag.cloneNodes().map(n=>[n.id,n])),done=new Map(),failed=new Map(),inFlight=new Map(),repairs=new Map(),buffer=input.eventBuffer||new BoundedEventBuffer({limit:input.eventLimit||1024,spill:input.spillEvents}),emit=async(type,data={})=>{const row={runId,type,at:now(),...clone(data)};await buffer.push(row);this.journal.append(runId,type,data);if(input.onEvent)await input.onEvent(clone(row));};await emit('RUN_STARTED',{nodes:pending.size});const initialPressure=typeof input.pressure==='function'?input.pressure():input.pressure||{},decision=this.policy.decide({...initialPressure,userMax:input.maxParallel,mobile:input.mobile}),semaphore=new AsyncSemaphore(decision.concurrency);let cancelled=false;
 const startNode=node=>{const p=semaphore.run(async()=>{if(input.signal?.aborted)return{nodeId:node.id,result:{state:'cancelled',reason:'CANCELLED'}};await emit('NODE_STARTED',{nodeId:node.id,capabilityId:node.capabilityId,criticality:rank.get(node.id)||0});const result=await this._executeNode(node,input);return{nodeId:node.id,result};},input.signal).catch(error=>({nodeId:node.id,result:{state:'failed',reason:String(error?.message||error)}}));inFlight.set(node.id,p);};
 while(pending.size||inFlight.size){if(input.signal?.aborted){cancelled=true;for(const[id]of pending)failed.set(id,{state:'cancelled',reason:'CANCELLED'});pending.clear();break;}for(const[id,node]of[...pending])if(node.dependsOn.some(d=>failed.has(d))){pending.delete(id);failed.set(id,{state:'blocked',reason:'DEPENDENCY_FAILED'});await emit('NODE_BLOCKED',{nodeId:id,reason:'DEPENDENCY_FAILED'});}if(typeof input.pressure==='function'){const current=this.policy.decide({...input.pressure(),userMax:input.maxParallel,mobile:input.mobile});semaphore.setLimit(current.concurrency);}
 const ready=[...pending.values()].filter(n=>n.dependsOn.every(d=>done.has(d))),ordered=planner.order(ready.map(x=>x.id),rank);for(const id of ordered){if(inFlight.size>=semaphore.limit)break;const node=pending.get(id);if(!node)continue;pending.delete(id);startNode(node);}if(!inFlight.size){if(pending.size)throw new Error('DAG_STALLED');break;}
 const settled=await Promise.race([...inFlight.values()]),{nodeId,result}=settled;inFlight.delete(nodeId);const node=dag.nodes.get(nodeId)||null;if(result.state==='success'){done.set(nodeId,result);await emit('NODE_COMPLETED',{nodeId,latencyMs:result.latencyMs||null});if(input.onPartial)await input.onPartial({nodeId,result:clone(result),done:done.size,total:dag.nodes.size});}else{let repaired=false,count=repairs.get(nodeId)||0;if(count<this.maxRepairs&&typeof input.repair==='function'){const replacement=await input.repair(clone(node),clone(result),{done:Object.fromEntries(done),failed:Object.fromEntries(failed),runId});if(replacement){pending.set(nodeId,{...clone(node),...clone(replacement),id:nodeId,dependsOn:clone(replacement.dependsOn||node.dependsOn)});repairs.set(nodeId,count+1);await emit('NODE_REPAIRED',{nodeId,attempt:count+1,reason:result.reason||result.state});repaired=true;}}if(!repaired){failed.set(nodeId,result);await emit('NODE_FAILED',{nodeId,state:result.state,reason:result.reason||null});}}
 if((done.size+failed.size)>0&&(done.size+failed.size)%Math.max(1,Number(input.checkpointEvery||4))===0)this.journal.checkpoint(runId,{done:Object.fromEntries(done),failed:Object.fromEntries(failed),pending:[...pending.keys()]});}
 if(cancelled&&inFlight.size){const drain=Promise.allSettled([...inFlight.values()]);await Promise.race([drain,new Promise(r=>setTimeout(r,Math.max(1,Number(input.cancelDrainMs||50))))]);}const state=cancelled?'cancelled':failed.size?'partial':'success',out={state,runId,done:Object.fromEntries(done),failed:Object.fromEntries(failed),repairs:Object.fromEntries(repairs),concurrency:decision,events:buffer.snapshot(),bulkheads:this.bulkheads.snapshot()};this.journal.checkpoint(runId,out);await emit('RUN_FINISHED',{state,done:done.size,failed:failed.size});return out;}
}
