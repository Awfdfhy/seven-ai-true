...
(async()=>{
 ...
 os.createProject({id:'seven_project',name:'Seven Ultimate'});os.projects.addChat('seven_project',{id:'chat1',title:'Build'});os.projects.attachFile('seven_project',{id:'spec',name:'spec.md',sourceRef:'repo:spec'});os.projects.createArtifact('seven_project',{id:'art1',name:'Prototype',content:'v1'});os.projects.artifacts.commit('art1',{content:'v2'});
 os.evals.runner.history.push({suiteId:'smoke',subjectId:'runtime',overall:1,categories:{core:1},avgLatencyMs:10,rows:[]});os.evals.promotion.register({id:'candidate1'});os.evals.promptEvolution.add({id:'prompt1',template:'base'});os.media.configure('image',true);
 ...
 const saved=await os.save('state',0);assert.equal(saved.revision,1);os.core.appendEvent('valen','main','prime',{id:'temporary'});os.plots.create({id:'temporary_plot'});os.projects.create({id:'temporary_project'});assert.equal(os.core.gameSummary('valen').events,3);
 ...
 assert.equal(os.projects.projects.has('seven_project'),true,'project workspace must persist');assert.equal(os.projects.projects.has('temporary_project'),false,'post-save projects must not survive restore');assert.equal(os.projects.artifacts.get('art1').versions.length,2,'artifact versions must persist');assert.equal(os.evals.runner.history.length,1,'eval history must persist');assert.equal(os.evals.promotion.get('candidate1').status,'quarantined','promotion state must persist');assert.equal(os.evals.promptEvolution.variants.has('prompt1'),true,'prompt variants must persist');assert.equal(os.media.isEnabled('image'),true,'optional media preference must persist');
 assert.equal(os.theme({mode:'night'}).launcherIcon,'seven-night');assert.equal(os.snapshot().tools,41);assert.equal(os.snapshot().projects,1);
 console.log('ultimate runtime: 32 assertions PASS');
