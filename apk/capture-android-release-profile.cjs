...
const EVIDENCE_AUTHORITY="ai.seven.app.test.sevenevidence";
...
function extractPrivate(fileName,local){
  if(!/^[a-z0-9-]+\.png$/.test(fileName))throw Error(`invalid evidence filename:${fileName}`);
  const uri=`content://${EVIDENCE_AUTHORITY}/${fileName}`;
  const r=spawnSync("adb",["exec-out","content","read","--uri",uri],{encoding:null,maxBuffer:32*1024*1024});
  if(r.error||r.status!==0)throw Error(`adb content evidence export failed for ${fileName}: ${r.error?.message||String(r.stderr||r.stdout||"")}`);
  if(!Buffer.isBuffer(r.stdout)||r.stdout.length<24)throw Error(`private evidence export empty:${fileName}`);
...
function captureProfile({apk,testApk,build,profileId,outDir,runId="local",environmentType="EMULATOR"}){
  ...
  for(const s of SHOTS){const local=path.join(shotDir,s.file);extractPrivate(s.file,local);pngSize(local)}
...
module.exports=Object.freeze({SHOTS,DEFAULT_RELEASE_APK,DEFAULT_TEST_APK,DEFAULT_BUILD_IDENTITY,TEST_PACKAGE,EVIDENCE_AUTHORITY,fileHash,pngSize,lastPair,lastNumber,inferProfileId,captureProfile,main});
