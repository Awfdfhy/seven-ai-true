...
function captureProfile({apk,testApk,build,profileId,outDir,runId="local",environmentType="EMULATOR"}){
  ...
  adb("shell","rm","-rf","/sdcard/Android/data/ai.seven.app.test/files/seven-visual");
  const inst=adb("shell","am","instrument","-w","-r","-e","class","ai.seven.app.SevenVisualEvidenceTest","ai.seven.app.test/androidx.test.runner.AndroidJUnitRunner");
  ...
  for(const s of SHOTS){const remote=`/sdcard/Android/data/ai.seven.app.test/files/seven-visual/${s.file}`,local=path.join(shotDir,s.file);adb("pull",remote,local);pngSize(local)}
