...
function buildReviewSkeleton(record){
  const stages={};for(const stage of tournament.STAGES){let status="CAPTURE_REQUIRED";if(stage==="HUMAN_EVIDENCE")status="OPTIONAL_WHERE_AVAILABLE";else if(["DISTINCTIVENESS","MOTION_MARK","SIMPLIFIER"].includes(stage))status="INDEPENDENT_REVIEW_REQUIRED";stages[stage]={status,evidence:[]}}
