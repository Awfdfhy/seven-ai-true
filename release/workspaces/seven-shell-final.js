...
const S={version:'3.0.0',ready:false,observer:null,workspaceLoading:null,attachmentWarm:false,events:false};
...
function polishComposer(){
  const tools=$('.composer-tools');if(tools){tools.setAttribute('role','toolbar');tools.setAttribute('aria-label',T('Message tools','أدوات الرسالة'));}
  const attach=$('[data-seven-attach-trigger],.composer-tools .tool-btn:not(.toggle)');if(attach){attach.classList.add('seven-shell-attach-trigger');attach.setAttribute('aria-label',T('Attach photos or files','إرفاق صور أو ملفات'));attach.title=T('Attach photos or files','إرفاق صور أو ملفات');}
  const mode=$('.seven-mode-trigger');if(mode){mode.setAttribute('aria-label',T('Response mode','نمط الإجابة'));mode.title=T('Response mode','نمط الإجابة');}
  const send=$('#sendBtn,.input-area .send');if(send){send.setAttribute('aria-label',T('Send message','إرسال الرسالة'));send.title=T('Send message','إرسال الرسالة');}
}
...
function polishSettings(){
  qa('.settings-modal,.settings-panel,.modal').forEach(m=>{m.dataset.sevenShellSurface='settings';m.setAttribute('data-seven-shell-polished','1')});
  qa('.settings-modal input,.settings-modal select,.settings-modal textarea,.settings-panel input,.settings-panel select,.settings-panel textarea,.modal input,.modal select,.modal textarea').forEach(x=>x.classList.add('seven-shell-field'));
}
function polishWorkspaces(){qa('.seven-ws-launcher,.seven-workspace-root').forEach(x=>x.dataset.sevenShellIntegrated='1');qa('.seven-ws-choice').forEach(x=>x.setAttribute('data-seven-shell-card','1'));}
...
let pending=false;function sync(){if(pending)return;pending=true;queueMicrotask(()=>{pending=false;mark();cleanChrome();ensureNav();ensureWorkspaceChip();polishComposer();polishSettings();polishWorkspaces();ensureCodeCopy();syncNav()})}
function boot(){if(S.ready){sync();return S}mark();installEvents();sync();if(d.body&&!S.observer){S.observer=new MutationObserver(sync);S.observer.observe(d.body,{childList:true,subtree:true,attributes:true,attributeFilter:['class','hidden','aria-expanded']})}S.ready=true;d.dispatchEvent(new CustomEvent('seven:shellfinalready',{detail:{version:S.version}}));return S}
