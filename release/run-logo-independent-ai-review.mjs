...
const PREPROCESSING=Object.freeze({
  schema:'seven-logo-independent-review-preprocessing.v2',
  ...
  threshold:0.50,
  rule:'crop preserves the original HOST raster pixels; no semantic gate threshold is lowered'
});
const PREPROCESSING_SHA=tournament.sha(PREPROCESSING);
const REVIEWER_CONTEXT=`clean-room:${MODEL}@${MODEL_REVISION}:transformers.js-${TRANSFORMERS_VERSION}:wave14-v2:preprocess-${PREPROCESSING_SHA.slice(0,16)}`;
...
function markSizeFor(stage,variant){
  if(stage==='TINY_SIZE')return pxFromVariant(variant)||24;
  if(stage==='ADAPTIVE_MASK'||stage==='SILHOUETTE'||stage==='MONOCHROME'||stage==='DAY_NIGHT')return 108;
  return null;
}
function isolatedStage(stage){return PREPROCESSING.isolatedStages.includes(stage)}
async function preprocessPackage(pkg,{candidateId,stage,variant,forceIsolated=false}={}){
  ...
  const markSize=markSizeFor(stage,variant)||108;
  const cropSize=stage==='TINY_SIZE'?Math.max(markSize+PREPROCESSING.tinyPaddingPx,PREPROCESSING.tinyMinimumCropPx):PREPROCESSING.isolatedNormalCropPx;
  ...
  const dir=path.join(INPUT_DIR,safe(candidateId));fs.mkdirSync(dir,{recursive:true});
  const out=path.join(dir,`${safe(stage)}-${safe(variant)}-${cropSize}px.png`);
...
async function evaluateCandidate({classifier,record,hostCandidate,reviewCandidate}){
  ...
  const identityPkg=packageFor(hostCandidate,'DAY_NIGHT','day');
  const identityInput=await preprocessPackage(identityPkg,{candidateId:record.candidate.id,stage:'DAY_NIGHT',variant:'day',forceIsolated:true});
  const conceptRows=await classify(classifier,identityInput.path,[familyPrompt(record.candidate),'an unrelated generic abstract app symbol']);const conceptP=byLabel(conceptRows,familyPrompt(record.candidate));traces.concept={artifactSha256:identityPkg.binding.artifactSha256,labels:conceptRows,positive:familyPrompt(record.candidate),preprocessing:identityInput.meta};
  const distinctRows=await classify(classifier,identityInput.path,LANDSCAPE.prompts);const uniquePrompt=LANDSCAPE.prompts.at(-1),uniqueP=byLabel(distinctRows,uniquePrompt),known=distinctRows.filter(x=>x.label!==uniquePrompt),maxKnown=Math.max(...known.map(x=>x.score));const suspicious=maxKnown>=0.45&&maxKnown>uniqueP+0.15;traces.distinctiveness={artifactSha256:identityPkg.binding.artifactSha256,labels:distinctRows,uniquePrompt,uniqueP,maxKnown,suspicious,preprocessing:identityInput.meta};
...
async function main(){
  ...
  fs.rmSync(INPUT_DIR,{recursive:true,force:true});fs.mkdirSync(INPUT_DIR,{recursive:true});
