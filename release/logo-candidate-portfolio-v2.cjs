...
const seal=body=>Object.freeze({...body,seal:tournament.sha(body)});

const DEFINITIONS=Object.freeze([
  Object.freeze({id:"orbit-cut",family:"L-C",name:"Orbit Cut",concept:"Seven Cut carried by a restrained orbital thread; speed and cognition without a generic chatbot bubble.",bounds:{x:22,y:22,width:64,height:64},pathCount:3,nodeCount:20,body:'<path d="M25 59C25 40 38 27 56 27C68 27 78 33 83 43" fill="none" stroke="__P__" stroke-width="6" stroke-linecap="round"/><path d="M31 33H79L49 82" fill="none" stroke="__S__" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"/><circle cx="80" cy="44" r="4" fill="__P__"/>'}),
  Object.freeze({id:"infinite-cut",family:"L-A",name:"Infinite Cut",concept:"A single continuity loop interrupted by a decisive Seven Cut; open-ended work with a sharp identity break.",bounds:{x:22,y:22,width:64,height:64},pathCount:2,nodeCount:24,body:'<path d="M25 58C33 42 43 38 52 49L61 60C69 70 79 67 83 57C87 47 80 38 70 38C59 38 53 48 47 59L34 82" fill="none" stroke="__P__" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/><path d="M31 31H79L50 81" fill="none" stroke="__S__" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/>'}),
  Object.freeze({id:"eclipse-seven-v2",parentId:"eclipse-seven",family:"L-B",name:"Eclipse Seven II",concept:"A dominant Seven spine with one quiet eclipse arc; stronger tiny-size identity and mask breathing room while preserving Day/Night duality.",bounds:{x:26,y:24,width:58,height:62},pathCount:2,nodeCount:10,masterSecondary:"#0b1728",body:'<path d="M30 31H77L51 80" fill="none" stroke="__S__" stroke-width="10" stroke-linecap="round" stroke-linejoin="round"/><path d="M72 29A29 29 0 0 1 79 70" fill="none" stroke="__P__" stroke-width="7" stroke-linecap="round"/>'}),
  Object.freeze({id:"horizon-fold-v2",parentId:"horizon-fold",family:"L-D",name:"Horizon Fold II",concept:"A bold Seven fold crossed by one restrained horizon; fewer strokes, stronger optical occupancy, and a cleaner tiny-size silhouette.",bounds:{x:25,y:25,width:58,height:60},pathCount:2,nodeCount:8,masterSecondary:"#0b1728",body:'<path d="M29 32H78L51 80" fill="none" stroke="__S__" stroke-width="10" stroke-linecap="round" stroke-linejoin="round"/><path d="M27 57H61" fill="none" stroke="__P__" stroke-width="8" stroke-linecap="round"/>'}),
  Object.freeze({id:"state-node-seven-v2",parentId:"state-node-seven",family:"L-E",name:"State Node Seven II",concept:"A bold Seven route with a single state node integrated into the diagonal; system identity without a fragile constellation of dots.",bounds:{x:24,y:24,width:60,height:62},pathCount:2,nodeCount:9,masterSecondary:"#0b1728",body:'<path d="M29 32H78L51 80" fill="none" stroke="__S__" stroke-width="10" stroke-linecap="round" stroke-linejoin="round"/><circle cx="62" cy="61" r="7" fill="__P__"/>'}),
...
const svgWrap=body=>`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 108 108" width="108" height="108">${body}</svg>`;
function renderSvg(def,variant){
  if(variant==="BACKGROUND")return svgWrap('<rect x="0" y="0" width="108" height="108" fill="#07111f"/>');
  const base=PALETTES[variant];if(!base)throw new Error(`unknown variant ${variant}`);
  const p={...base};if(variant==="MASTER"&&def.masterSecondary)p.s=def.masterSecondary;
  return svgWrap(def.body.replaceAll("__P__",p.p).replaceAll("__S__",p.s));
}
const geometrySignature=def=>sha(def.body.replaceAll("__P__","#").replaceAll("__S__","#"));
...
function createRecord(def){
  ...
  const assets=VARIANTS.map(([kind,file,variant])=>assetRef(def,kind,file,variant));for(const a of assets)if(a.inspection.verdict!=="PASS")throw new Error(`${def.id}/${a.file} failed brand inspection: ${a.inspection.failures.join(",")}`);
  ...
  const candidate=tournament.createCandidate({id:def.id,family:def.family,name:def.name,concept:def.concept,genomeSeal:snapshot.seal,masterAsset:master,geometry,parentId:def.parentId||null});
...
function buildReviewSkeleton(record){const stages={};for(const stage of tournament.STAGES){let status="CAPTURE_REQUIRED";if(stage==="HUMAN_EVIDENCE")status="OPTIONAL_WHERE_AVAILABLE";else if(["DISTINCTIVENESS","MOTION_MARK","SIMPLIFIER"].includes(stage))status="INDEPENDENT_REVIEW_REQUIRED";stages[stage]={status,evidence:[]}}return {candidateId:record.candidate.id,candidateSeal:record.candidate.seal,stages,dimensions:Object.fromEntries(tournament.DIMENSIONS.map(d=>[d,null])),winnerEligible:false}}
function buildPortfolio(){
  const records=DEFINITIONS.map(createRecord),masterHashes=records.map(r=>r.assets.find(a=>a.kind==="MASTER_VECTOR").ref.sha256),geometryHashes=records.map(r=>r.geometrySignature),families=records.map(r=>r.candidate.family);
  if(new Set(masterHashes).size!==records.length)throw new Error("candidate master SVGs must be unique");if(new Set(geometryHashes).size!==records.length)throw new Error("candidate geometry signatures must be unique");if(new Set(families).size!==records.length)throw new Error("candidate tournament families must be unique");
