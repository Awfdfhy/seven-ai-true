...
async function collectStructure(page,scenario){
  return page.evaluate((scenario)=>{
    ...
    const presentSelectors=scenario.criticalSelectors.filter(s=>!!document.querySelector(s));
...
(async()=>{
  ...
  async function shot(page,fileName,input){
    ...
    const ev=V.createEvidence(scenario,artifact,{tier:'HOST',commitSha,branch,audits,policy:'OBSERVE',notes:['Wave 12 foundation: structural + screenshot evidence; warnings/failures are preserved rather than auto-approved.']});
  ...
  try{
    ...
    await shot(p,'01-chat-night.png',{id:'chat-night',surface:'core-chat',journey:'chat-idle',state:'idle',theme:'night',locale:'en',direction:'ltr',criticalSelectors:['#chat','.composer','.seven-beta-status'],tags:['phone','core','night']});
    await p.evaluate(()=>SevenTheme.setPreference('day'));await shot(p,'02-chat-day.png',{id:'chat-day',surface:'core-chat',journey:'chat-idle',state:'idle',theme:'day',locale:'en',direction:'ltr',criticalSelectors:['#chat','.composer','.seven-beta-status'],tags:['phone','core','day']});
    await p.evaluate(()=>{SevenTheme.setPreference('night');SevenAurora.set('thinking','medium')});await shot(p,'03-aurora-thinking.png',{id:'chat-thinking',surface:'core-chat',journey:'reasoning-state',state:'thinking',theme:'night',locale:'en',direction:'ltr',criticalSelectors:['#chat','.composer','.seven-beta-status'],tags:['phone','semantic-state']});
    await p.click('.seven-beta-status');await p.waitForFunction(()=>window.SevenWorkspaces&&document.querySelector('.seven-ws-launcher'));await shot(p,'04-workspace-launcher.png',{id:'workspace-launcher',surface:'workspace-launcher',journey:'open-workspace',state:'launcher',theme:'night',locale:'en',direction:'ltr',criticalSelectors:['.seven-ws-launcher','.seven-beta-status'],tags:['phone','workspace']});
    await p.click('[data-ws="coding"]');await p.waitForFunction(()=>window.SevenCodingWorkspace&&SevenWorkspaces.active()==='coding');await p.evaluate(()=>SevenCodingWorkspace.bindProject([{path:'src/main.js',content:"export function greet(name){\n  return `Hello ${name}`;\n}\n"},{path:'README.md',content:'# Seven Demo\nVerified local project context.'}]));await shot(p,'05-coding-agent.png',{id:'coding-agent',surface:'coding',journey:'project-bound',state:'ready',theme:'night',locale:'en',direction:'ltr',criticalSelectors:['.seven-beta-status'],tags:['phone','specialist','coding']});
    ...
    await shot(p,'06-rpg-real-works.png',{id:'rpg-real-works',surface:'rpg-real-works',journey:'canon-loaded',state:'ready',theme:'night',locale:'en',direction:'ltr',criticalSelectors:['.seven-beta-status'],tags:['phone','specialist','rpg']});
    await p.evaluate(()=>{document.documentElement.dir='rtl';document.documentElement.lang='ar'});await shot(p,'07-rpg-rtl-probe.png',{id:'rpg-rtl-probe',surface:'rpg-real-works',journey:'rtl-layout-probe',state:'ready',theme:'night',locale:'ar',direction:'rtl',criticalSelectors:['.seven-beta-status'],tags:['phone','rtl','structural-probe']});
    await c.close();
    ...
    await shot(rp,'08-chat-reduced-motion.png',{id:'chat-reduced-motion',surface:'core-chat',journey:'reduced-motion',state:'coding',theme:'night',locale:'en',direction:'ltr',reducedMotion:true,criticalSelectors:['#chat','.composer','.seven-beta-status'],tags:['phone','accessibility','reduced-motion']});await reduced.close();

    const manifest=V.createManifest({branch,commitSha,mode:'OBSERVE',scenarios,artifacts,evidence});fs.writeFileSync(path.join(out,'visual-evidence.json'),JSON.stringify(manifest,null,2));
