(()=>{
  ...
  function upgradeThemeButtons(root=document){
    root.querySelectorAll('button[onclick*="toggleTheme"]').forEach(btn=>{
      ...
      const orb=document.createElement('span');orb.className='seven-celestial-toggle';orb.setAttribute('aria-hidden','true');btn.prepend(orb);
    });
  }

  function placePersistenceStatus(){
    const status=document.getElementById('persistenceStatus');
    const meta=document.querySelector('.seven-composer-meta');
    if(!status||!meta)return;
    if(status.parentElement!==meta)meta.append(status);
    if(status.className!=='seven-save-status')status.className='seven-save-status';
    status.removeAttribute('style');
  }

  function syncProcessingState(){
    const stop=document.getElementById('stopBtn');if(!stop)return;
  ...
  function watchProcessing(){
    const stop=document.getElementById('stopBtn');if(!stop||stop.dataset.sevenObserved==='1')return;
    stop.dataset.sevenObserved='1';new MutationObserver(syncProcessingState).observe(stop,{attributes:true,attributeFilter:['style','class','hidden']});syncProcessingState();
  }

  function refresh(){renderContextRail();upgradeThemeButtons();placePersistenceStatus();watchProcessing();}

  let refreshPending=false;
  function scheduleRefresh(){if(refreshPending)return;refreshPending=true;(window.requestAnimationFrame||((fn)=>setTimeout(fn,16)))(()=>{refreshPending=false;refresh()});}
  const observer=new MutationObserver(records=>{if(records.some(record=>record.addedNodes?.length||record.type==='attributes'))scheduleRefresh();});
