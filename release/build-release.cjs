...
function build(){
  ...
  const fingerprint=digest(css+canon+world+research+performance+control+pdfRuntime+motion+ui+pdf.version);
  ...
  fs.writeFileSync(path.join(DIST_DIR,'release-manifest.json'),JSON.stringify({format:'seven-release-manifest',version:7,builtAt:new Date().toISOString(),...result},null,2));
  return result;
}
