...
function verifyIndependentAiReview(x,{portfolio,hostPack,reviewPack}){
  if(!x||x.schema!=="seven-logo-independent-ai-review.v1"||!HASH64.test(String(x.seal||"")))return false;const {seal,...body}=x;if(seal!==tournament.sha(body))return false;
...
function chooseTradeoff(adjudication,portfolio){
  ...
  // Constitution/brief order: geometry/recognition first; no weighted scalar is allowed.
