...
const report={format:'seven-static-audit',version:12,sourceBytes:Buffer.byteLength(sourceHtml),builtHtmlBytes:built.bytes,releaseLayerBytes:layerBytes,themeBootBytes:built.themeBootBytes,pdfVendorBytes:built.pdf.bytes,pdfLoadMode:built.pdfLoadMode,staticPackageBytes,apkStaticBudgetBytes,assets,issues,warnings,apkReadiness:{ready:apkBlockers.length===0,blockers:apkBlockers}};
...
console.log(`static audit: PASS (${layerBytes} release bytes incl ${built.themeBootBytes} theme boot, ${staticPackageBytes}/${apkStaticBudgetBytes} static APK bytes, lazy-local PDF, ${warnings.length} warnings)`);
