...
const report={format:'seven-static-audit',version:3,sourceBytes:Buffer.byteLength(sourceHtml),builtHtmlBytes:built.bytes,releaseLayerBytes:layerBytes,pdfVendorBytes:built.pdf.bytes,staticPackageBytes,apkStaticBudgetBytes,assets,issues,warnings,apkReadiness:{ready:apkBlockers.length===0,blockers:apkBlockers}};
...
console.log(`static audit: PASS (${layerBytes} release bytes, ${staticPackageBytes}/${apkStaticBudgetBytes} static APK bytes, ${warnings.length} warnings)`);
