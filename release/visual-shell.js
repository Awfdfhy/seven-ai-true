(()=>{
  ...
  function upgradeSidebar(){
    ...
    words.className='seven-brand-wordmark';
    words.innerHTML='<strong>Seven</strong><small>Intelligence System</small>';
    lockup.append(words);
  ...
  function upgradeTopbar(){
    ...
    identity.className='seven-top-identity';
    identity.append(mark('top',false));
    const text=document.createElement('div');
    text.innerHTML='<strong>Seven</strong><br><span data-seven-mode-label>Core</span>';
    identity.append(text);
    if(menu && menu.nextSibling) topbar.insertBefore(identity,menu.nextSibling); else topbar.prepend(identity);

    const dock=document.createElement('nav');
    dock.className='seven-mode-dock';
    dock.setAttribute('aria-label','Seven modes');
    Object.entries(MODES).forEach(([id,mode])=>{
      const btn=document.createElement('button');
      btn.type='button';
      btn.className='seven-mode-button';
      btn.dataset.mode=id;
      btn.title=mode.hint;
    ...
    const gear=topbar.querySelector('.icon-btn');
    if(gear) topbar.insertBefore(dock,gear); else topbar.append(dock);
  }
  ...
  function decorateEmptyState(){
    ...
    if(!identity){
      ...
      identity.className='seven-empty-identity';
      const stage=document.createElement('div');
      stage.className='seven-empty-orbit-stage';
      stage.append(mark('hero',true));
      identity.append(stage);
      const greeting=empty.querySelector('.empty-greeting');
      if(greeting) empty.insertBefore(identity,greeting); else empty.prepend(identity);
    }
    let kicker=empty.querySelector('.seven-mode-kicker');
    if(!kicker){
      kicker=document.createElement('div');
      kicker.className='seven-mode-kicker';
  ...
  function upgradeComposer(){
    ...
    meta.className='seven-composer-meta';
    meta.innerHTML='<span>Local state ready</span><span><b data-seven-mode-label>Core</b> · adaptive runtime</span>';
  ...
  function sectionizeSettings(){
    ...
    const originalTitle=modal.querySelector(':scope > h3');
    const actions=modal.querySelector(':scope > .modal-buttons');

    const head=document.createElement('div');
    head.className='seven-settings-head';
    head.append(mark('modal',false));
    const headText=document.createElement('div');
    headText.innerHTML='<h3>Seven Settings</h3><p>Models, memory, knowledge and local runtime controls</p>';
    head.append(headText);
    modal.insertBefore(head,modal.firstChild);

    const grid=document.createElement('div');
    grid.className='seven-settings-grid';
    if(originalTitle?.nextSibling) modal.insertBefore(grid,originalTitle.nextSibling); else modal.append(grid);

    const definitions={
      intelligence:['Intelligence','Routing'],
      providers:['Providers','Free only'],
      generation:['Generation','Response'],
      memory:['Memory & Knowledge','Context'],
      data:['Data & Appearance','Local']
    };
    const sections={};
    for(const [key,[title,badge]] of Object.entries(definitions)){
      const section=document.createElement('section');
      section.className='seven-settings-section';
      section.dataset.section=key;
      if(key==='memory'||key==='data') section.dataset.span='2';
      const heading=document.createElement('div');
      heading.className='seven-settings-section-title';
      heading.innerHTML=`<span>${title}</span><small>${badge}</small>`;
    ...
    for(const node of candidates){
      ...
      if(text==='Free Providers') bucket='providers';
      else if(node.tagName==='LABEL' && /^Temperature\b/i.test(text)) bucket='generation';
      else if(node.tagName==='LABEL' && /^Pinned Notes/i.test(text)) bucket='memory';
      else if(node.classList?.contains('settings-row') && /Export/i.test(text)) bucket='data';
  ...
  function upgradeNameModal(){
    const modal=document.querySelector('#nameModal .modal-content');
    if(!modal || modal.dataset.sevenBrand==='1') return;
    modal.dataset.sevenBrand='1';
    const old=modal.querySelector('img.name-modal-logo');
    if(old){
      const holder=mark('hero',false);
      holder.style.width='82px';holder.style.height='82px';holder.style.margin='0 auto 12px';
  ...
  function upgradeThemeMotion(){
    ...
    function wrapped(...args){
      document.documentElement.classList.remove('seven-theme-shift');
      void document.documentElement.offsetWidth;
      document.documentElement.classList.add('seven-theme-shift');
      const result=original.apply(this,args);
      setTimeout(()=>document.documentElement.classList.remove('seven-theme-shift'),650);
      return result;
    }
    wrapped.__sevenWrapped=true;
