(()=>{
  ...
  const MODES={
    core:{label:'Core',short:'Core',hint:'General intelligence',placeholder:'Message Seven…',sub:'Ask, plan, create, or make something happen.',icon:'<circle cx="12" cy="12" r="7"/><circle cx="12" cy="12" r="2"/>'},
    build:{label:'Build',short:'Build',hint:'Coding and agents',placeholder:'Ask Seven to build, inspect, or debug…',sub:'Inspect code, run the loop, verify the result.',icon:'<path d="M8 9 4 12l4 3M16 9l4 3-4 3M14 5l-4 14"/>'},
    world:{label:'World',short:'World',hint:'RPG and canon',placeholder:'Enter the world…',sub:'A stateful world where choices leave consequences.',icon:'<path d="M12 3a9 9 0 1 0 9 9M3.6 9h16.8M3.6 15h10.7M12 3c2.1 2.3 3.2 5.3 3.2 9 0 1.1-.1 2.1-.3 3M12 21c-2.1-2.3-3.2-5.3-3.2-9S9.9 5.3 12 3"/>'},
    research:{label:'Research',short:'Research',hint:'Evidence and sources',placeholder:'Ask a research question…',sub:'Trace claims, evidence, conflicts, and uncertainty.',icon:'<circle cx="10.5" cy="10.5" r="5.5"/><path d="m15 15 5 5M8 10.5h5M10.5 8v5"/>'}
  };
  ...
  function mark(size='top',animate=false){
    const host=document.createElement('span');host.className='seven-brand-mark';host.dataset.size=size;host.dataset.animate=animate?'true':'false';host.innerHTML=MARK;
    const orbit=host.querySelector('ellipse');if(orbit)orbit.classList.add('seven-orbit');return host;
  }
  ...
  function setMode(mode,{persist=true,announce=true}={}){
    ...
    document.querySelectorAll('.seven-mode-button').forEach(btn=>{const next=btn.dataset.mode===mode?'true':'false';if(btn.getAttribute('aria-selected')!==next)btn.setAttribute('aria-selected',next);});
    document.querySelectorAll('[data-seven-mode-label]').forEach(node=>writeText(node,MODES[mode].label));
    const input=document.getElementById('userInput');if(input&&input.placeholder!==MODES[mode].placeholder)input.placeholder=MODES[mode].placeholder;
    decorateEmptyState();
    if(announce)document.dispatchEvent(new CustomEvent('seven:modechange',{detail:{mode}}));
  }

  function upgradeSidebar(){
    const header=document.querySelector('.sidebar-header');if(!header||header.dataset.sevenUpgraded==='1')return;
    header.dataset.sevenUpgraded='1';header.querySelector('img.app-icon')?.remove();header.querySelector('.app-name')?.remove();
    const lockup=document.createElement('div');lockup.className='seven-brand-lockup';lockup.append(mark('sidebar',false));
    const words=document.createElement('div');words.className='seven-brand-wordmark';words.innerHTML='<strong>Seven</strong><small>Intelligence System</small>';lockup.append(words);header.append(lockup);
  }

  function upgradeTopbar(){
    const topbar=document.querySelector('.topbar');if(!topbar||topbar.dataset.sevenUpgraded==='1')return;topbar.dataset.sevenUpgraded='1';
    const menu=topbar.querySelector('.menu-toggle');const identity=document.createElement('div');identity.className='seven-top-identity';identity.append(mark('top',false));
    ...
    Object.entries(MODES).forEach(([id,mode])=>{const btn=document.createElement('button');btn.type='button';btn.className='seven-mode-button';btn.dataset.mode=id;btn.title=mode.hint;btn.setAttribute('aria-selected','false');btn.setAttribute('aria-label',mode.label+' mode');btn.innerHTML=svgIcon(mode.icon)+`<span>${mode.short}</span>`;btn.addEventListener('click',()=>setMode(id));dock.append(btn);});
  ...
  function decorateEmptyState(){
    ...
    if(!identity){identity=document.createElement('div');identity.className='seven-empty-identity';const stage=document.createElement('div');stage.className='seven-empty-orbit-stage';stage.append(mark('hero',true));identity.append(stage);const greeting=empty.querySelector('.empty-greeting');if(greeting)empty.insertBefore(identity,greeting);else empty.prepend(identity);}
    let kicker=empty.querySelector('.seven-mode-kicker');if(!kicker){kicker=document.createElement('div');kicker.className='seven-mode-kicker';kicker.innerHTML='<span data-seven-mode-label></span><span>workspace</span>';empty.append(kicker);}
    const mode=currentMode();writeText(kicker.querySelector('[data-seven-mode-label]'),MODES[mode].label);
    const sub=empty.querySelector('.empty-sub');if(sub)writeText(sub,MODES[mode].sub);
  }
  ...
  function upgradeNameModal(){
    const modal=document.querySelector('#nameModal .modal-content');if(!modal||modal.dataset.sevenBrand==='1')return;modal.dataset.sevenBrand='1';const old=modal.querySelector('img.name-modal-logo');
  ...
  function upgradeThemeMotion(){
    if(typeof window.toggleTheme!=='function'||window.toggleTheme.__sevenWrapped)return;const original=window.toggleTheme;
  ...
  function boot(){
    if(window.SevenVisualShell)return;upgradeSidebar();upgradeTopbar();upgradeComposer();sectionizeSettings();upgradeNameModal();decorateEmptyState();upgradeThemeMotion();ensureAccessibleLabels();setMode(currentMode(),{persist:false,announce:false});
    document.documentElement.dataset.sevenVisual='brand-os-v1';document.documentElement.classList.add('seven-visual-ready');window.SevenVisualShell=Object.freeze({version:1,setMode,currentMode,refresh:refreshDerivedUI});
    const observer=new MutationObserver(records=>{if(records.some(record=>record.addedNodes&&record.addedNodes.length))scheduleRefresh();});observer.observe(document.body,{subtree:true,childList:true});
    document.dispatchEvent(new CustomEvent('seven:visualready',{detail:{version:1,mode:currentMode()}}));
  }

  function foundationState(){try{return typeof roomPersistence!=='undefined'?roomPersistence.status():null;}catch(_){return null;}}
  function hydrateAfterFoundation(){
    const state=foundationState();if(state&&(state.ready||state.failed)){boot();return;}const status=document.getElementById('persistenceStatus');
    if(!status){if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',hydrateAfterFoundation,{once:true});else setTimeout(hydrateAfterFoundation,0);return;}
    const observer=new MutationObserver(()=>{const next=foundationState();if(next&&(next.ready||next.failed)){observer.disconnect();boot();}});observer.observe(status,{childList:true,characterData:true,subtree:true});
    const raced=foundationState();if(raced&&(raced.ready||raced.failed)){observer.disconnect();boot();}
  }
