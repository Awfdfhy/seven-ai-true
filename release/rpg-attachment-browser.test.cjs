...
(async()=>{
  ...
  try{
    ...
    await p.waitForFunction(()=>SevenWorkspaces.active()==='rpg'&&window.SevenRpgWorkspace);
    const cold=await p.evaluate(()=>({workspace:SevenWorkspaces.active(),attachmentReady:!!window.SevenAttachments?.state?.ready,inputVisible:!document.querySelector('.input-area').hidden}));
    assert.deepEqual(cold,{workspace:'rpg',attachmentReady:false,inputVisible:true},'RPG navigation must keep attachment runtime cold');
    const coldTrigger='.composer-tools .tool-btn:not(.toggle)';
    assert.equal(await p.isVisible(coldTrigger),true,'RPG attachment intent target must remain available while runtime is cold');
    await p.click(coldTrigger);
    await p.waitForFunction(()=>window.SevenAttachments?.state?.ready&&document.querySelector('[data-seven-attach-trigger]')?.getAttribute('aria-expanded')==='true'&&!document.querySelector('.seven-attach-menu')?.hidden,{timeout:10000});
    ...
    assert.equal(await p.isVisible('.seven-attach-menu'),true,'RPG paperclip intent must lazy-load runtime and open on the first tap');
    await p.keyboard.press('Escape');
    assert.equal(await p.getAttribute('[data-seven-attach-trigger]','aria-expanded'),'false');
    assert.deepEqual(errors,[]);
    await c.close();
    console.log('RPG attachment lazy first-tap Android-browser test: PASS');
