...
match(mat,/EVIDENCE_ROOT="\/data\/local\/tmp\/seven-visual"/,"instrumentation must stage screenshots in shell-owned test evidence storage independent of app sandbox availability");
match(mat,/rm -rf .*EVIDENCE_ROOT.*mkdir -p .*EVIDENCE_ROOT/,"instrumentation must clear stale shell evidence before capture");
match(mat,/screencap -p /,"screenshots must be genuine Android system screencaps");
match(mat,/wc -c < /,"instrumentation must fail closed on empty staged captures");
...
console.log(`Android Release Capture Contract: PASS (${n} assertions; release-variant instrumentation, shell-owned screencap staging, signer parity and truthful visual evidence)`);
