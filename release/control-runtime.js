(function(root,factory){
  ...
  function createClaim(input){
    ...
    return {schemaVersion:1,id:String(input.id||id('claim')),text,kind:String(input.kind||'CLAIM').toUpperCase(),authority:effective,sources,lineage:clone(input.lineage||{parents:[],transformation:'direct'}),contradicts:strings(input.contradicts),createdAt:input.createdAt||new Date().toISOString()};
  }
  function deriveClaim(input){input=input||{};const parents=arr(input.parents);if(!parents.length)throw new Error('derived claim requires parents');return createClaim({id:input.id,text:input.text,kind:input.kind||'INFERENCE',authority:weakest(parents.map(p=>p.authority)),sources:parents.flatMap(p=>arr(p.sources)),lineage:{parents:parents.map(p=>p.id),transformation:input.transformation||'derived',transformer:input.transformer||null}});}
  function resolveClaim(claim,options){options=options||{};if(!claim)return {state:'UNKNOWN',reason:'missing-claim',authority:'NONE'};if(arr(claim.contradicts).length)return {state:'CONFLICT',reason:'explicit-conflict',authority:claim.authority};
    const groups=new Set(arr(claim.sources).map(s=>s.independentGroup||s.id));if(Number(options.minIndependentSources||0)>groups.size)return {state:'UNKNOWN',reason:'insufficient-independent-sources',authority:claim.authority};
    ...
    if(!arr(claim.sources).length&&claim.kind!=='ASSUMPTION')return {state:'UNKNOWN',reason:'no-sources',authority:'NONE'};return {state:claim.kind,reason:'supported',authority:claim.authority};}
  ...
  function compileContext(input){input=input||{};const max=Math.max(0,Math.floor(Number(input.maxTokens)||0));const reserve=Math.max(0,Math.floor(Number(input.reserveTokens)||0));const available=Math.max(0,max-reserve);const shares={...DEFAULT_SHARES,...(input.categoryShares||{})};const sum=Object.values(shares).reduce((a,b)=>a+Math.max(0,Number(b)||0),0)||1;const budgets=Object.fromEntries(Object.entries(shares).map(([k,v])=>[k,Math.floor(available*(Math.max(0,Number(v)||0)/sum))]));const usage=Object.fromEntries(Object.keys(budgets).map(k=>[k,0]));const selected=[],evicted=[],warnings=[];let total=0;
    ...
    eligible.sort((a,b)=>((b.pinned?10000:0)+(b.required?5000:0)+Number(b.priority||0))-((a.pinned?10000:0)+(a.required?5000:0)+Number(a.priority||0)));
    for(const item of eligible){const required=item.pinned||item.required,catFits=usage[item.category]+item.tokens<=(budgets[item.category]||0),fits=total+item.tokens<=available;if(fits&&(required||catFits)){selected.push(item);usage[item.category]+=item.tokens;total+=item.tokens;if(required&&!catFits)warnings.push('category-overflow:'+item.category);}else evicted.push({...item,evictionReason:fits?'category-budget':'total-budget'});}return {selected,evicted,tokensUsed:total,tokenBudget:available,categoryUsage:usage,categoryBudgets:budgets,warnings};}
  ...
  return {VERSION,state,TASK_STATES,AUTH,TIERS,createTaskContract,canUseCapability,transitionTask,createClaim,deriveClaim,resolveClaim,compileContext,tokensOf,selectTier,createBudget,adaptBudget,createSideEffectLedger,boot,snapshot};
});
