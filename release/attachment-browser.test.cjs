...
(async()=>{
  ...
  try{
    ...
    const base=await page.evaluate(()=>({version:SevenAttachments.version,accept:fileInput.accept,multiple:fileInput.multiple,menu:document.querySelectorAll(".seven-attach-menu").length,photos:document.querySelectorAll('[data-seven-attach="photos"]').length,files:document.querySelectorAll('[data-seven-attach="files"]').length,style:!!document.getElementById("seven-attachment-style"),cats:[SevenAttachments.category(new File(["x"],"photo.webp",{type:"image/webp"})),SevenAttachments.category(new File(["x"],"pack.zip",{type:"application/zip"})),SevenAttachments.category(new File(["x"],"main.py",{type:"text/plain"})),SevenAttachments.category(new File(["x"],"deck.pptx",{type:"application/vnd.openxmlformats-officedocument.presentationml.presentation"}))]}));
    assert.equal(base.version,"1.2.0-beta.1");assert.equal(base.accept,"*/*");assert.equal(base.multiple,true);assert.equal(base.menu,1);assert.equal(base.photos,1);assert.equal(base.files,1);assert.equal(base.style,true);assert.deepEqual(base.cats,["image","archive","text","presentation"]);
    ...
    await context.close();
    console.log("Universal attachment Android-browser test: PASS");
