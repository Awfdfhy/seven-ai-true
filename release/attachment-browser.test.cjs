...
(async()=>{
  ...
  try{
    ...
    await page.evaluate(()=>{const m=document.getElementById("nameModal");if(m){m.style.setProperty("display","none","important");m.hidden=true;m.setAttribute("aria-hidden","true");m.style.pointerEvents="none"}document.querySelectorAll('.modal').forEach(x=>{if(x.id==='nameModal'){x.classList.remove('show','open','active');}})});
    await page.waitForFunction(()=>{const m=document.getElementById('nameModal');return !m||getComputedStyle(m).display==='none'||getComputedStyle(m).pointerEvents==='none'});
    const trigger=page.locator('.composer-tools .tool-btn:not(.toggle)').first();
    ...
    await context.close();
    console.log("Universal attachment Android-browser test: PASS (trusted pointerdown lazy intent load)");
