...
(async()=>{
  ...
  try{
    ...
    await page.waitForFunction(()=>window.SevenPerformance&&window.SevenPerformance.state.ready&&window.SevenControl&&window.SevenControl.state.ready&&window.SevenBridge&&window.SevenBridge.state.ready&&window.SevenCanon&&window.SevenWorld&&window.SevenResearch&&window.SevenMotion&&window.SevenMotion.state.ready&&window.SevenUI&&window.SevenUI.state.ready&&window.SevenPdf,null,{timeout:10000});
    ...
    await test('control bridge boots and preserves resource tier',async()=>{const r=await page.evaluate(()=>({ready:SevenBridge.state.ready,version:SevenBridge.VERSION,tier:SevenBridge.syncResources().tier,control:SevenControl.state.tier,performance:SevenPerformance.state.tier}));assert.equal(r.ready,true);assert.equal(r.version,'1.0.0');assert.equal(r.tier,r.control);assert.equal(r.control,r.performance)});
    ...
    await test('research gaps remain epistemically unresolved',async()=>{const r=await page.evaluate(()=>{const verification=SevenResearch.verify([{id:'supported',text:'supported'},{id:'gap',text:'gap'}],[{id:'source',title:'Source',url:'https://example.com/source',authority:'A1',evidence:[{claimId:'supported',stance:'support',excerpt:'e'}]}]);const truth=SevenBridge.researchVerificationToTruth(verification);return {status:truth.status,supported:truth.claims.find(c=>c.id==='research:supported').kind,gap:truth.claims.find(c=>c.id==='research:gap').kind,unresolved:truth.unresolved};});assert.equal(r.status,'INCONCLUSIVE');assert.equal(r.supported,'FACT');assert.equal(r.gap,'UNKNOWN');assert.ok(r.unresolved.includes('research:gap'))});
    await test('real works cannot claim canon without source coverage',async()=>{const r=await page.evaluate(()=>{const e=SevenWorld.createEngine({id:'work',sources:[{id:'official',authority:'A0',url:'https://example.com/canon'}],beats:[{id:'verified',sourceRefs:['official']},{id:'gap',sourceRefs:[]}]});const s=e.createSession();const verified=SevenBridge.worldContractTruth(e.sceneContract(s,{beatId:'verified'}),e.work);const gapContract=e.sceneContract(s,{beatId:'gap'});const gap=SevenBridge.worldContractTruth(gapContract,e.work);const guard=SevenBridge.guardCanonCommit({contract:gapContract,work:e.work,commitResult:{status:'CANON'}});return {verified:verified.status,gap:gap.status,allowed:guard.allowed,guardStatus:guard.status};});assert.equal(r.verified,'PASS');assert.equal(r.gap,'CANON_GAP');assert.equal(r.allowed,false);assert.equal(r.guardStatus,'CANON_GAP')});
    await test('bridge compiles task truth memory and conversation under one budget',async()=>{const r=await page.evaluate(()=>{const task=SevenControl.createTaskContract({id:'ctx',goal:'answer',allowedCapabilities:['read']});const out=SevenBridge.buildTaskContext({task,truth:[{id:'truth',kind:'FACT',text:'known'}],memory:[{id:'deleted-memory',content:'bad',lifecycle:'deleted'}],conversation:[{id:'msg',content:'hello'}],maxTokens:1000});return {used:out.tokensUsed,budget:out.tokenBudget,ids:out.selected.map(x=>x.id)};});assert.ok(r.used<=r.budget);assert.ok(r.ids.includes('task:ctx'));assert.ok(!r.ids.includes('deleted-memory'))});
    ...
    await rp.waitForFunction(()=>window.SevenPerformance&&SevenPerformance.state.ready&&window.SevenControl&&SevenControl.state.ready&&window.SevenBridge&&SevenBridge.state.ready&&window.SevenUI&&SevenUI.state.ready,null,{timeout:10000});
    await test('reduced motion forces lightweight motion and control tiers',async()=>{const r=await rp.evaluate(()=>({reduced:SevenPerformance.state.reducedMotion,tier:SevenPerformance.state.tier,controlTier:SevenControl.state.tier,bridgeTier:SevenBridge.syncResources().tier,attr:document.documentElement.dataset.sevenReducedMotion}));assert.equal(r.reduced,true);assert.equal(r.tier,'lite');assert.equal(r.controlTier,'lite');assert.equal(r.bridgeTier,'lite');assert.equal(r.attr,'1')});
    await reduced.close();
  } finally {
