...
for(const r of p.records){
  ...
  const review=portfolio.buildReviewSkeleton(r);eq(review.winnerEligible,false);eq(review.stages.DISTINCTIVENESS.status,"INDEPENDENT_REVIEW_REQUIRED");eq(review.stages.MOTION_MARK.status,"INDEPENDENT_REVIEW_REQUIRED");eq(review.stages.HUMAN_EVIDENCE.status,"OPTIONAL_WHERE_AVAILABLE");eq(review.stages.TINY_SIZE.status,"CAPTURE_REQUIRED");
}
