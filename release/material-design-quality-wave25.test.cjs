...
// The committed plan is part of the gate, not optional prose.
for(const token of [
...
// Day/Night are real paired implementations, not a single inverted palette.
...
// Governed geometry/elevation/focus system.
...
// Full material is intentionally concentrated in high-value surfaces.
...
// Material governor must degrade cost, never capability.
...
// Reduced Motion is independent and removes motion-dependent spectacle.
...
// Mobile ergonomics and safe areas remain part of premium quality.
...
// Design Lint must be completely clean on both live release stylesheets.
...
// Product quality cannot be separated from real wiring and state completeness.
...
for(const row of matrix.rows){
  ...
  ok(Array.isArray(row.applicable_states)&&row.applicable_states.includes('error'),'release-relevant capability lacks designed error state: '+row.capability_id);
}

// Cross-cutting proof files must exist and remain separate concerns.
for(const file of [
...
// Fancy material must remain restrained. These two stylesheets are small enough that runaway blur is a release smell.
const liveCss=css+'\n'+beta;
...
ok(backdropUses<=12,'backdrop-filter proliferation violates restraint/performance boundary');
ok(!/transition\s*:\s*all\b/i.test(liveCss),'transition:all is forbidden in final material system');

// Protected source remains immutable while release-layer material work evolves.
