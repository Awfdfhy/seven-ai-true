...
function adjudicateCandidateEntry({record,hostCandidate,reviewCandidate,reviewSubmission,hardGateFindings}){
  ...
  if(!handoffRuntime.verifyReviewHandoff(reviewCandidate.handoff,{portfolioManifest:null,candidate:undefined}))throw new Error(`review handoff invalid: ${candidate.id}`);
  if(reviewCandidate.handoff.candidateSeal!==candidate.seal)throw new Error(`review handoff candidate mismatch: ${candidate.id}`);
...
function adjudicateTournament({portfolio,hostPack,reviewPack,submissionsByCandidate,hardGateFindingsByCandidate}){
  ...
  const entries=[];
  for(const record of portfolio.records){const id=record.candidate.id;entries.push(adjudicateCandidateEntry({record,hostCandidate:hostById.get(id),reviewCandidate:reviewById.get(id),reviewSubmission:submissionsByCandidate[id],hardGateFindings:hardGateFindingsByCandidate[id]}));}
  ...
  const inconclusive=entries.filter(x=>!finalists.includes(x)&&!rejected.includes(x.candidateId)).map(x=>x.candidateId).sort();
  let comparison={verdict:"INCONCLUSIVE",reason:"no-finalist"},selectionState="NO_CHAMPION",eligible=[];
  if(finalists.length===1){comparison={verdict:"UNIQUE_SURVIVOR",undominated:[finalists[0].candidateId],dimensionFloors:{[finalists[0].candidateId]:finalists[0].baseVerdict.dimensionFloor}};selectionState="UNIQUE_SELECTION_AVAILABLE";eligible=[finalists[0].candidateId]}
...
function createChampionDecision({adjudication,selectedCandidateId,decider,deciderContext,reason,policy}){
  ...
  if(r.selectionState==="UNIQUE_SELECTION_AVAILABLE"){
    ...
    const builderContext=req(r.entries.length?"seven-ci-logo-builder-wave14-v1":"","builderContext");void builderContext;
...
function verifyChampionDecision(d,adjudication){
  try{if(!verifySeal(d,"seven-logo-champion-decision")||d.status!=="CHAMPION_SELECTED"||d.authorityBoundary?.brandFreezeStillBlocked!==true)return false;if(adjudication){if(!verifyTournamentAdjudication(adjudication)||d.adjudicationSeal!==adjudication.receipt.seal||d.portfolioSeal!==adjudication.receipt.portfolioSeal||!adjudication.receipt.eligibleChampionIds.includes(d.selectedCandidateId))return false}return DECISION_POLICIES.includes(d.policy)}catch{return false}
}
