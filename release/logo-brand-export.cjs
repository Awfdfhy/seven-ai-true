...
function materializeBrandExport({root=process.cwd(),portfolio,adjudication,decision,exporter}={}){
  ...
  try{
    if(previousDigest){fs.mkdirSync(rollbackRoot,{recursive:true});if(fs.existsSync(backupDir))fs.rmSync(backupDir,{recursive:true,force:true});fs.renameSync(finalDir,backupDir);backedUp=true}
    else fs.rmSync(finalDir,{recursive:true,force:true});
    ...
    fs.writeFileSync(path.join(finalDir,"brand-export-manifest.json"),JSON.stringify(manifest,null,2)+"\n");fs.writeFileSync(path.join(finalDir,"consumption-plan.json"),JSON.stringify(consumptionPlan,null,2)+"\n");fs.writeFileSync(path.join(finalDir,"champion-decision.json"),JSON.stringify(decision,null,2)+"\n");
    const finalDigest=dirDigest(finalDir),receipt=seal({schema:"seven-logo-brand-materialization",version:VERSION,candidateId:plan.record.candidate.id,candidateSeal:plan.record.candidate.seal,championDecisionSeal:decision.seal,exportPlanSeal:plan.receipt.seal,brandManifestSeal:manifest.seal,consumptionPlanSeal:consumptionPlan.seal,previousDigest,backupPath:backupDir?path.relative(root,backupDir).split(path.sep).join("/"):null,finalDigest,status:"MATERIALIZED_NOT_FROZEN",authorityBoundary:{rollbackSnapshotPreserved:!!previousDigest,androidConsumptionStillRequired:true,releaseBuildDeviceEvidenceStillRequired:true,brandFreezeStillBlocked:true}});fs.writeFileSync(path.join(finalDir,"materialization-receipt.json"),JSON.stringify(receipt,null,2)+"\n");
