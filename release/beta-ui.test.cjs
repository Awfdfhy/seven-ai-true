...
(async()=>{
  ...
  const server=http.createServer((req,res)=>{
    ...
    if(pathname.startsWith('/vendor/')||pathname.startsWith('/brand/')||pathname.startsWith('/workspaces/')){
      ...
      const type=file.endsWith('.css')?'text/css; charset=utf-8':file.endsWith('.js')||file.endsWith('.mjs')?'text/javascript; charset=utf-8':file.endsWith('.svg')?'image/svg+xml':'application/octet-stream';
  ...
  try{
    ...
    await page.waitForFunction(()=>window.SevenBetaUI&&SevenBetaUI.state.ready&&window.SevenAurora&&window.SevenShellFinal&&SevenShellFinal.state.ready,{timeout:10000});
    await page.evaluate(()=>{deepThinkToggle.classList.remove('active');searchToggle.classList.remove('active');SevenBetaUI.syncMode();SevenShellFinal.sync();});
    const state=await page.evaluate(()=>({
      ...
      shellReady:SevenShellFinal.state.ready,
    ...
    assert.equal(state.rootClass,true);assert.equal(state.badgeCount,0);assert.equal(state.statusCount,1);assert.equal(state.badgeText,undefined);assert.equal(state.statusText,'Chat');assert.equal(state.shellReady,true);
    ...
    await page.evaluate(()=>{SevenBetaUI.sync();SevenShellFinal.sync();});
    ...
    assert.deepEqual(idempotent,{badge:0,status:1,picker:1},'final shell cleanup and beta sync must be idempotent');
    ...
    const ap=await android.newPage();const androidErrors=[];ap.on('pageerror',e=>androidErrors.push(e.message));await ap.goto(origin,{waitUntil:'domcontentloaded'});await ap.waitForFunction(()=>window.SevenBetaUI&&SevenBetaUI.state.ready&&window.SevenShellFinal&&SevenShellFinal.state.ready,{timeout:10000});
    ...
    await reducedPage.waitForFunction(()=>window.SevenBetaUI&&SevenBetaUI.state.ready&&window.SevenAurora&&window.SevenShellFinal&&SevenShellFinal.state.ready,{timeout:10000});
    ...
    await reduced.close();
    console.log('beta UI + final shell + modes + stop + Aurora browser tests: PASS');
